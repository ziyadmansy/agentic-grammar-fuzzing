...
57.6\% to 96.8\% relative to a static baseline, using no coverage data at
all. Structural fingerprints and rejection signatures are reported as
descriptive signals; because the refined arm executes more iterations, their
aggregate counts are not treated as like-for-like effects. We additionally
run the identical, unmodified pipeline against a second parser (parson) for five real
